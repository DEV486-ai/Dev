...
def change_password():
    ...
    if session.get("is_admin"):
        ...
        # Check old password matches admin password
        # We read it from the file itself
        if old_pwd != ADMIN_PASSWORD:
            return jsonify({"ok":False,"msg":"❌ বর্তমান Password ভুল"})
        if len(new_pwd) < 6:
            return jsonify({"ok":False,"msg":"❗ কমপক্ষে ৬ অক্ষর দিন"})
        
        # Update in file
        try:
            with open(__file__, 'r') as f:
                file_content = f.read()
            old_line = f'ADMIN_PASSWORD = "{ADMIN_PASSWORD}"'
            new_line = f'ADMIN_PASSWORD = "{new_pwd}"'
            ...
            return jsonify({"ok":False,"msg":"File update error: "+str(ex)})
    ...
    c=db()
    row=c.execute("SELECT id FROM users WHERE id=? AND pwd=?",
        (session["uid"], sha(old_pwd))).fetchone()
    if not row:
        c.close()
        return jsonify({"ok":False,"msg":"❌ বর্তমান Password ভুল"})
    
    c.execute("UPDATE users SET pwd=? WHERE id=?", (sha(new_pwd), session["uid"]))
    c.commit(); c.close()
    return jsonify({"ok":True})
...
def change_password():
    ...
    d=request.json
    old=d.get("old_pwd","").strip()
    new=d.get("new_pwd","").strip()
    if not old or not new: return jsonify({"ok":False,"msg":"সব ফিল্ড পূরণ করুন"})
    if len(new)<6: return jsonify({"ok":False,"msg":"নতুন Password কমপক্ষে ৬ অক্ষর"})
    uid=session["uid"]
    # Admin password change
    if session.get("is_admin"):
        return jsonify({"ok":False,"msg":"Admin password code এ বদলাতে হবে"})
    c=db()
